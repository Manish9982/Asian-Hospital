...
const Signup = ({ navigation, route }) => {
  ...
  const { Ncity, Nstate, Ncountry, NmobileNo, NsignedState } = useContext(DataContext)
      ...
      <View>
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            setHospitalCode(item.organizationcode)
            setData2({
              "hospital": {
                "name": `${item.name}`,
  ...
  const handleChange = async () => {
    ...
    if (!((salutation == "") || (firstName == "") || (dob == "") || (city == "") || (country == "") || (state == "") || (!validateEmail(email)) || (mobileNo == "") || (gender == "") || (flatNo == "") || (hospitalCode == ""))) {
      ...
      try {
        const response = await fetch(`${Constants.API_BASE}register`, requestOptions)
        ...
        if (result.status == '200') {
          ...
          setLoader(true)
          savelocalStorageData('token', result?.token)
    ...
    if (hospitalCode == "") {
      Alert.alert("Info", "Please choose a hospital")
    }
              ...
              {data2?.hospital?.name == null ?
                ...
                <>
                  <Text style={{
                    ...
                    fontFamily: fontFamily.medium,
                  }}>{data2?.hospital?.name}</Text>
                  <View style={{
                    flexDirection: "row",
                    paddingHorizontal: W * 0.05
                  }}>
                    {data2?.hospital?.address1 &&
                      <Image
                        style={{
                          height: H * 0.025,
                          width: H * 0.025,
