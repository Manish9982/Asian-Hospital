...
const SearchCountry = ({ navigation }) => {
    ...
    const { Ncity, Ncountry } = useContext(DataContext)
    ...
    const getCityNames = async () => {
        const result = await GetApiData('countries')
        // console.log(result)
        if (result.status == '200') {
            setData(result)
            setFilteredCityName(result?.countries)
        } else {
    ...
    const filterSearch = (text) => {
        if (text !== "") {
            ...
            setSearchValue(text);
            setFilteredCityName(data?.countries)
        }
    }
